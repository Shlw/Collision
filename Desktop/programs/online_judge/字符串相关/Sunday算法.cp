...
int strStr(const char *source, const char *target) {
    ...
    int slen = (int)strlen(source), tlen = (int)strlen(target);
    if (slen < tlen) return -1; if (!tlen) return 0;
    int lstApr[128]; memset(lstApr, -1, 128 * sizeof(int));
    int i, ptr = 0;
    for (i = 0; i < tlen; ++i) lstApr[target[i]] = i;
    for (i = 0; ptr + tlen <= slen;) {
        if (source[ptr + i] == target[i]) {
            if (i == tlen - 1) return ptr;
            ++i;
        } else {
            i = lstApr[source[ptr + tlen]];
            if (i == -1) {
                ptr = ptr + tlen + 1;
            } else {
                ptr = ptr + tlen - i;
