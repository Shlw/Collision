...
int main() {
...
#endif // ONLINE_JUDGE
    char c[40]; scanf("%s", c);
    bool a[10] = {0}; int len = strlen(c) - 1, t = c[len]-'0', r = 0;
    if (t == 0 || t == 5) a[5] = true;
    if (len>0) { t += 10*(c[len-1]-'0'); if (len>1) t += 100*(c[len-2]-'0'); }
    if (t%2==0) {
        a[2] = true;
        if (t%4==0) {
            a[4] = true;
    ...
    int mdzz[]={1,3,2,-1,-3,-2}, tt = 0, rr = 0;
    for (int i = len; i >= 0; --i, ++tt) {
        if (tt > 5) tt -= 6;
        rr += (c[i]-'0')*mdzz[tt];
    }
    if (rr%7==0) a[7] = true;
    bool eat_space = true;
    for (int i = 2; i < 10; ++i) if (a[i]) if (eat_space){ printf("%d", i); eat_space = false; } else printf(" %d", i);
    if (eat_space) printf("none");
    printf("\n");
#ifndef ONLINE_JUDGE
    COUNT(the program);
#endif // ONLINE_JUDGE
