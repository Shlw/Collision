...
int r, c, m[MAX_LEN][MAX_LEN], max_l[MAX_LEN][MAX_LEN] = {0};
int rst = 0;

int dfs(int x, int y) {
    if (max_l[x][y]) return max_l[x][y];
    max_l[x][y] = 1;
    int x1, y1, i;
    FIN(i, 4) {
        x1 = x + dir[i][0]; y1 = y + dir[i][1];
        if (x1 >= 0 && y1 >= 0 && x1 < r && y1 < c && m[x1][y1] < m[x][y]) {
            rst = max(rst, max_l[x][y] = max(1 + dfs(x1, y1), max_l[x][y]));
        }
    }
    return (max_l[x][y] = max(1, max_l[x][y]));
}

int main() {
    cin >> r >> c;
    int i, j;
    FIN(i, r) FIN(j, c) cin >> m[i][j];
    FIN(i, r) FIN(j, c) dfs(i, j);
    cout << max(rst, 1) << endl;
}
