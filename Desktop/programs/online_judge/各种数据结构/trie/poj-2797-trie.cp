...
public:
    ...
    node *getParent();
    void addNode(char *);
    void travelTrie(node *, string, string);
};
...
void trie::travelTrie(node *start, string word, string pre) {
    if (NULL == start) return;
    if (start->occupied) prefix[word] = pre.length() ? pre : word;
    for (int i = 0; i < MAX_N; ++i) {
        string p = pre;
        if (!pre.length() && start->count == 1) {
            p = word;
        }
        travelTrie(start->next[i], word + (char)(i+'a'), p);
    }
}
