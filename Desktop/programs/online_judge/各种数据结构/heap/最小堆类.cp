...
public:
    ...
    ~Minheap();
    void insert(T);
    void insert(T, int (* _Nonnull)(const T, const T));
    T del();
    T del(int (* _Nonnull)(const T, const T));
...
void Minheap<T>::insert(T data) {
    if (this->size >= MAX_LEN - 1) throw "no enough memory";
    int i = ++size;
    // heap[i / 2] is heap[i]'s parent
    while (this->heap[i / 2] > data && i != 1) {
...
template <class T>
void Minheap<T>::insert(T data, int (* _Nonnull comp)(const T, const T)) {
    if (this->size >= MAX_LEN - 1) throw "no enough memory";
    int i = ++size;
    while (comp(this->heap[i / 2], data) || i == 1) {
        this->heap[i] = this->heap[i / 2];
        i /= 2;
    }
    this->heap[i] = data;
...
template <class T>
T Minheap<T>::del() {
    if (!size) throw "empty heap";
    // root
    T rst = this->heap[1], lst = this->heap[size--];
    int i = 2;
    while (i <= this->size) {
        // heap[i + 1] may do not exist
        if (i < this->size && this->heap[i + 1] < this->heap[i]) ++i;
        if (this->heap[i] > lst) break; // ???
    ...
    // clear
    T zero = T();
    this->heap[size + 1] = zero;
    return rst;
}

// to copy!
template <class T>
T Minheap<T>::del(int (* _Nonnull comp)(const T, const T)) {
    T rst = this->heap[1];
    int i = 2;
    while (i < this->size) {
        if (comp(this->heap[i], this->heap[i + 1])) {
            this->heap[i / 2] = this->heap[i];
            i *= 2;
        } else {
            this->heap[i / 2] = this->heap[i + 1];
            i = 2 * i + 2;
        }
    }
    return rst;
