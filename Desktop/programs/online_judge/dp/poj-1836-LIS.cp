...
#elif defined poj_1836
#define INF 0x3f3f3f3f
int main() {
    INITICK;
    TICK;
    
    int n, i, j, r=INF, dp[1111]={1}, dp2[1111]={1};
    double a[1111]={0}; scanf("%d",&n);
    for (i = 0; i < n; ++i) scanf("%lf",a+i);
    // LIS
    // binary search can give O(nlogn), but i'm so lazy...
    for (i = 1; i < n; ++i) {
        dp[i] = 1;
        for (j = 0; j < i; ++j)
            if (a[j] < a[i]) dp[i] = (dp[i] <= dp[j] ? dp[j]+1 : dp[i]);
    }
    for (i = 1; i < n; ++i) dp[i] = (dp[i] < dp[i-1] ? dp[i-1] : dp[i]);
    // reverse LIS
    dp2[n-1] = 1;
    for (i = n-2; i >= 0; --i) {
        dp2[i] = 1;
        for (j = n-1; j > i; --j)
            if (a[j] < a[i]) dp2[i] = (dp2[i] <= dp2[j] ? dp2[j]+1 : dp2[i]);
    }
    for (i = n-2; i >= 0; --i) dp2[i] = (dp2[i] < dp2[i+1] ? dp2[i+1] : dp2[i]);
    // note that it's unnecessary to compare n-dp[i]-dp2[i] with r
    // because dp2[i+1] <= dp2[i]
    for (i = 0; i < n-1; ++i)
        if (n-dp[i]-dp2[i+1] < r) r = n-dp[i]-dp2[i+1];
    printf("%d\n", r);
